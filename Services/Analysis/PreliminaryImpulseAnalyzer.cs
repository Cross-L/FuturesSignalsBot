...
public static class PreliminaryImpulseAnalyzer
{
        ...
        IEnumerable<PreliminaryImpulse> preliminaryImpulses, int topCount, bool byLow)
    {
        var sortedImpulses = byLow
            ? preliminaryImpulses.OrderBy(impulse => impulse.ChangeOv).Take(topCount).ToList()
            : preliminaryImpulses.OrderByDescending(impulse => impulse.ChangeOv).Take(topCount).ToList();
